...
CREATE INDEX `categoryId_idx` ON `bookstore`.`book` (`categoryID` ASC) VISIBLE;

CREATE INDEX `publisherfk2_idx` ON `bookstore`.`book` (`PubID` ASC) VISIBLE;

CREATE INDEX `price_index` ON `bookstore`.`book` (`price` ASC) VISIBLE;
...
CREATE INDEX `authorName_idx` ON `bookstore`.`book_authors` (`AuthorID` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `bookstore`.`authors` (
  ...
  CONSTRAINT `authorname`
    ...
    REFERENCES `bookstore`.`book_authors` (`AuthorID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 601
...
CREATE INDEX `ISBNinOrders` ON `bookstore`.`ordertable` (`ISBN` ASC) VISIBLE;

CREATE INDEX `PublisherId_idx` ON `bookstore`.`ordertable` (`PublisherId` ASC) VISIBLE;
...
CREATE INDEX `BOOK_idx` ON `bookstore`.`topsales` (`ISBN` ASC) VISIBLE;

CREATE INDEX `USER_idx` ON `bookstore`.`topsales` (`userID` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- procedure Retreive_unconfirmed_orders
-- -----------------------------------------------------

DELIMITER $$
USE `bookstore`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Retreive_unconfirmed_orders`()
BEGIN
select Name , ISBN from (publisher inner  join (select ISBN,PublisherId  from ordertable where confirmed=false) as tab1 on tab1.PublisherId=publisher.PublisherID) ;
...
set @pubID = (select PubID from book where ISBN =new.ISBN);
    insert into ordertable (PublisherId,ISBN,QUANTITY,confirmed) values (@pubID,new.ISBN,new.threshold-new.quantity+100,false);
 END if;
...
-- -----------------------------------------------------
-- Event to delete the old records from topsales table every month
-- -----------------------------------------------------
